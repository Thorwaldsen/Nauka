...
def preview_records():
    ...
    if len(records) == 0:
        print("\nUnfortunetly there are no passwords in the Vault.")
        return False
...
def generate_new_password(master_key):
    ...
    while True:
        n_numbers = input_as_digit(input(prompt[0]), prompt[0], 0, 10)
        n_letters = input_as_digit(input(prompt[1]), prompt[1], 0, 10)
        n_special_chars = input_as_digit(input(prompt[2]), prompt[2], 0, 10)
        ...
        print(
            ...
            "3. Discard and return to Main Menu"
        )
...
def main_menu(master_key):
    while True:
        ...
        if user_input == "1":
            ...
            print("Your passwords are save. Have a nice day.")
            exit()
        else:
            print("invalid input. Try again")
...
print("\nGreetings Master! Please provide the Master Key to preceed!")
